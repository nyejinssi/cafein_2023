...
const Tmp = () => {
  ...
  const DataInput = async (e) => {
    ...
    try {
      for (const item of selectedItems) {
        ...
        await addDoc(collection(dbService, "PaymentCheck"), PaymentData);
      }
  ...
  <Col lg="3" md="3" sm="1" xs="1">
        ...
        <div className="myPageCart">
          ...
          <li style={{ margin: 0, paddingLeft: 0 }}>
            <div className="form-check form-check-inline" id="chkAll">
              <input
 ...
 <ul className="cartList">
      ...
      <li style={{ margin: 0, paddingLeft: 0 }}>
        ...
        <li className="quantityConts">{userCartItem.countNumber}</li>
        <li className="amountConts">
        {userCartItem.ProductPrice} <br /></li>
        <button name="delete" onClick={() => handleDelete(userCartItem.id)}> 삭제 </button>
        </ul>
...
<table className="cartCalc">
          <tbody>
            <tr>
            <td style={{ borderRight: "none" }}>최종 결제 금액(Cafe인은 전 상품 배송비 무료): {userPrice}</td>
            </tr>
  </tbody>
  </table>
